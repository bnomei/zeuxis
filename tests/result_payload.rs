...
use zeuxis::mcp::{
    ...
    tools::{
        ...
        DiagnoseRuntimeParams, GetLatestScreenshotParams, ListMonitorsParams,
    },
};
...
#[tokio::test]
async fn result_payload_capture_window_at_cursor_propagates_cursor_failure() {
    let harness = create_test_harness();
...
#[tokio::test]
async fn result_payload_diagnose_runtime_reports_component_failures_without_tool_error() {
    let harness = create_test_harness();
    *harness.permission.result.lock().expect("lock") =
...
#[tokio::test]
async fn result_payload_diagnose_runtime_reports_monitor_worker_panic() {
    let harness = create_test_harness();
